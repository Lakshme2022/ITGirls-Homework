...
class Heroes {
    constructor(name, universe, alterego, occupation, friends, superpowers, url, info, index, rating=0) {
        this.name = name;
    ...
    getCard() {
        return `<div class="card">
                <div class="info">
                <h2>${this.name}</h2>
                <p>Вселенная: ${this.universe}</p>
                <p>Альтер эго: ${this.alterego}</p>
                <p>Род деятельности: ${this.occupation}</p>
                <p>Друзья: ${this.friends}</p>
                <p>Суперсилы: ${this.superpowers}</p>
                </div>
                <img class="cards__img" src="${this.url}">
                <div class="rating-area">
                    <input type="radio" name='rating${this.id}' ${(this.rating === 5 ? 'checked' : '')} value="5">
                    <label onclick="setRating(5, ${this.id})" for="5"></label>
                    <input type="radio" name="rating${this.id}" ${(this.rating === 4 ? 'checked' : '')} value="4">
                    <label onclick="setRating(4, ${this.id})" for="4" ></label>    
                    <input type="radio" name="rating${this.id}" ${(this.rating === 3 ? 'checked' : '')} value="3">
                    <label onclick="setRating(3, ${this.id})" for="3"></label>  
                    <input type="radio" name="rating${this.id}" ${(this.rating === 2 ? 'checked' : '')} value="2">
                    <label onclick="setRating(2, ${this.id})" for="2" ></label>    
                    <input type="radio" name="rating${this.id}" ${(this.rating === 1 ? 'checked' : '')} value="1">
                    <label onclick="setRating(1, ${this.id})" for="1" ></label>               
                </div>
...
let heroesObjects = data.map((hero, index) => new Heroes(hero.name, hero.universe, hero.alterego, hero.occupation, hero.friends, hero.superpowers, hero.url, hero.info, index));
const content = document.getElementById('content');

function render() {
    content.innerHTML = "";
    heroesObjects.forEach(hero => content.innerHTML += hero.getCard());

}
function setRating(value, index) {
    heroesObjects[index].rating = value;
    render();
    saveToStorage();
}

function loadFromStorage() {
    let local_data = window.localStorage.getItem('dataJson');
    if (local_data) {
        return JSON.parse(local_data).map((hero, index) => new Heroes(hero.name, hero.universe, hero.alterego, hero.occupation, hero.friends, hero.superpowers, hero.url, hero.info, index, hero.rating));
    }
    return null;
}

function saveToStorage() {
    window.localStorage.setItem('dataJson', JSON.stringify(heroesObjects));
}
