...
function result_phrase(final_percent) {
    ...
    if (final_percent <= 59) {
        return 'Ты можешь смело искать предложения по стажировке, но повтори перед этим следующие темы, находящиеся по ссылкам ниже:';
    }
    if (60 <= final_percent <= 79) {
        return 'Перед составлением резюме рекомендую сходить по нижеприведенным ссылкам и повторить темы:'
    }
